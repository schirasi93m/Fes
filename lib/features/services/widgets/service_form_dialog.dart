...
class _ServiceDialogState extends State<ServiceDialog> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      setState(() {
        ...
        _selectedCustomer = widget.service == null
          ? null
          : _customers.cast<CustomerModel?>().firstWhere(
            (customer) => customer?.id == widget.service!.customerId,
            orElse: () => null,
            );
        _selectedExtinguisher = widget.service == null
          ? null
          : _extinguishers.cast<ExtinguisherModel?>().firstWhere(
            (extinguisher) =>
              extinguisher?.id == widget.service!.extinguisherId,
            orElse: () => null,
            );
        _isLoading = false;
      });
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final savedService = _isEditMode
          ? await _serviceApi.updateService(
              ServiceModel(
                id: widget.service!.id,
                customerId: service.customerId,
                extinguisherId: service.extinguisherId,
                serviceDate: service.serviceDate,
                nextServiceDate: service.nextServiceDate,
                description: service.description,
                needsValve: service.needsValve,
                needsGauge: service.needsGauge,
                needsPipe: service.needsPipe,
                needsPowder: service.needsPowder,
                needsHose: service.needsHose,
                isActive: service.isActive,
              ),
            )
          : await _serviceApi.createService(service);
  ...
  String _errorMessage(Object error) {
    if (error is DioException) {
      ...
      return 'خطای سرور در ثبت سرویس (کد ${response?.statusCode ?? 'نامشخص'}).';
    }

    return 'ثبت سرویس با خطا مواجه شد: $error';
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: Text(_isEditMode ? 'ویرایش سرویس' : 'سرویس جدید'),
